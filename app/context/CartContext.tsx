...
export type CartProduct = { id: string; name: string; price: number; image_url: string | null; max_quantity?: number };
...
export function CartProvider({children}: {children: ReactNode}) {
  ...
  useEffect(() => {let cancelled=false;queueMicrotask(() => {if(cancelled)return;try {const parsed:unknown = JSON.parse(localStorage.getItem("cosmic-cart") ?? "[]");if(Array.isArray(parsed)){const seen=new Set<string>();const cleaned:CartItem[]=[];for(const item of parsed.slice(0,40)){if(!item || typeof item !== "object")continue;const row=item as Partial<CartItem>;if(typeof row.id!=="string" || typeof row.name!=="string" || typeof row.price!=="number" || !Number.isFinite(row.price) || row.price<0 || seen.has(row.id))continue;seen.add(row.id);const product:CartProduct={id:row.id,name:row.name.slice(0,100),price:row.price,image_url:typeof row.image_url==="string"?row.image_url:null,max_quantity:typeof row.max_quantity==="number"?row.max_quantity:undefined};cleaned.push({...product,quantity:Math.max(1,Math.min(limit(product),Number.isInteger(row.quantity)?Number(row.quantity):1))});}setItems(cleaned);}}catch{localStorage.removeItem("cosmic-cart");}setCartLoaded(true);});return()=>{cancelled=true;};},[]);
