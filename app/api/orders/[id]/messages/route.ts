...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const {data:lastMessage}=await admin.from("order_messages").select("created_at").eq("order_id",id).eq("user_id",user.id).order("created_at",{ascending:false}).limit(1).maybeSingle();
  if(lastMessage&&Date.now()-new Date(lastMessage.created_at).getTime()<1500)return NextResponse.json({error:"Aguarde um instante antes de enviar novamente."},{status:429});
