...
const DesktopNav = ({ admin = false, discordUrl }: { admin?: boolean; discordUrl: string }) => <>
...
export default async function SiteHeader() {
  ...
  const nickname = user
    ? String(profile?.nickname ?? user.user_metadata?.preferred_username ?? user.user_metadata?.user_name ?? user.email?.split("@")[0] ?? "Minha conta")
    : "";
  ...
  return <header className="site-header sticky top-0 z-50 border-b border-white/[.08] bg-[#090811]/95 backdrop-blur-xl">
    <div className="shell flex h-[68px] items-center justify-between gap-3">
      <Link href="/" aria-label="Cosmic Store - início" className="flex shrink-0 items-center gap-2.5">
        <span className="relative h-10 w-10 overflow-hidden rounded-xl border border-violet-500/25">
          <Image src="/images/branding/cosmic-store-logo.png" alt="" fill sizes="40px" className="object-cover" priority />
        </span>
        <span className="hidden text-lg font-black tracking-[-.05em] min-[390px]:inline">COSMIC<span className="text-violet-400">.</span></span>
      </Link>

      <nav aria-label="Navegação principal" className="desktop-nav hidden items-center gap-1 text-sm font-semibold text-zinc-400 lg:flex">
        <DesktopNav admin={isAdmin} discordUrl={discordUrl}/>
      </nav>

      <div className="flex items-center gap-2">
        {user
          ? <AccountMenu name={nickname} avatarUrl={avatarUrl} isAdmin={isAdmin} discordUrl={discordUrl} />
          : <Link href="/login" className="hidden rounded-lg px-3 py-2 text-sm font-bold text-zinc-300 hover:text-white sm:block">Entrar</Link>}
        <CartButton />
        <HeaderMobileMenu loggedIn={Boolean(user)} discordUrl={discordUrl} />
      </div>
    </div>
  </header>;
}
