...
export default async function AdminOrdersPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  const unreadResult = user
    ? await supabase.from("notifications").select("link").eq("user_id", user.id).is("read_at", null).like("link", "/admin/pedidos/%")
  ...
  return <main className="min-h-screen bg-[#080812] p-4 text-white sm:p-6"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-4 grid gap-3 md:hidden">{orders.map((order) => { const status = orderStatus[order.status] ?? { label: order.status, className: "" }; const profile = Array.isArray(order.profiles) ? order.profiles[0] : order.profiles; const unreadCount = unreadByOrder.get(`/admin/pedidos/${order.id}`) ?? 0; return <Link key={order.id} href={`/admin/pedidos/${order.id}`} className={`rounded-2xl border p-4 ${unreadCount ? "border-violet-500/40 bg-violet-500/[.06]" : ["proof_submitted","under_review"].includes(order.status) ? "border-amber-400/30 bg-amber-500/5" : "border-white/10 bg-white/[.025]"}`}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><strong>{order.order_code}</strong>{unreadCount > 0 && <span className="rounded-full bg-red-500 px-2 py-0.5 text-[10px] font-black text-white">{unreadCount} nova{unreadCount === 1 ? "" : "s"}</span>}</div><p className="mt-1 text-xs text-zinc-400">{new Date(order.created_at).toLocaleString("pt-BR")}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${status.className}`}>{status.label}</span></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><span className="block text-xs text-zinc-500">Cliente</span>{profile?.nickname ?? "Cliente"}</div><div><span className="block text-xs text-zinc-500">Nickname</span>{order.game_nickname}</div></div><div className="mt-4 flex items-center justify-between border-t border-white/10 pt-3"><strong>{Number(order.total).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</strong><span className="font-bold text-violet-300">Abrir →</span></div></Link>; })}</div>
    <div className="mt-4 hidden overflow-x-auto rounded-2xl border border-white/10 md:block"><table className="w-full min-w-[780px] text-left text-sm"><thead className="bg-white/5 text-zinc-300"><tr><th className="p-4">Pedido</th><th className="p-4">Cliente</th><th className="p-4">Nickname</th><th className="p-4">Total</th><th className="p-4">Situação</th><th className="p-4"></th></tr></thead><tbody>{orders.map((order) => { const status = orderStatus[order.status] ?? { label: order.status, className: "" }; const profile = Array.isArray(order.profiles) ? order.profiles[0] : order.profiles; const unreadCount = unreadByOrder.get(`/admin/pedidos/${order.id}`) ?? 0; return <tr key={order.id} className={`border-t border-white/10 ${unreadCount ? "bg-violet-500/[.06]" : ["proof_submitted","under_review"].includes(order.status) ? "bg-amber-500/5" : ""}`}><td className="p-4 font-bold"><span className="inline-flex flex-wrap items-center gap-2">{order.order_code}{unreadCount > 0 && <span className="rounded-full bg-red-500 px-2 py-0.5 text-[10px] font-black text-white">{unreadCount} nova{unreadCount === 1 ? "" : "s"}</span>}</span><span className="mt-1 block text-xs font-normal text-zinc-400">{new Date(order.created_at).toLocaleString("pt-BR")}</span></td><td className="p-4">{profile?.nickname ?? "Cliente"}</td><td className="p-4">{order.game_nickname}</td><td className="p-4 font-bold">{Number(order.total).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</td><td className="p-4"><span className={`rounded-full px-3 py-1 text-xs font-bold ${status.className}`}>{status.label}</span></td><td className="p-4"><Link href={`/admin/pedidos/${order.id}`} className="font-bold text-violet-300">Abrir →</Link></td></tr>; })}</tbody></table></div>
