...
export default async function ReviewsAdminPage() {
  ...
  return <main className="admin-page"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">REPUTAÇÃO DA LOJA</p><h1 className="admin-title">Avaliações</h1><p className="admin-description">Gerencie avaliações verificadas do site e importe o histórico antigo do Discord.</p></div></div>
    ...
    <section className="admin-panel mt-6"><div className="flex items-center justify-between gap-4"><div><p className="eyebrow">AVALIAÇÕES</p><h2 className="mt-1 text-xl font-black">Últimas avaliações</h2></div><span className="text-xs text-zinc-500">Até 100 mais recentes</span></div><div className="mt-5 space-y-3">{reviews.map(review => <article key={review.id} className="rounded-xl border border-white/10 bg-black/15 p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong>{"★".repeat(review.rating)}</strong><span className={review.source === "discord" ? "rounded-full bg-indigo-500/10 px-2 py-1 text-[10px] font-bold text-indigo-200" : "rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-bold text-emerald-200"}>{review.source === "discord" ? "✓ Compra verificada • Discord" : "✓ Compra verificada"}</span>{!review.is_visible && <span className="rounded-full bg-zinc-500/10 px-2 py-1 text-[10px] font-bold text-zinc-400">Oculta</span>}</div><p className="mt-2 text-sm leading-6 text-zinc-300">“{review.comment}”</p><div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-zinc-500"><strong className="text-zinc-300">{review.author_nickname}</strong><span>{new Date(review.created_at).toLocaleDateString("pt-BR")}</span>{review.attachment_url && <a href={review.attachment_url} target="_blank" rel="noreferrer" className="font-bold text-violet-300">📷 Ver imagem da entrega ↗</a>}</div></div><form action={toggleReviewVisibility} className="shrink-0"><input type="hidden" name="id" value={review.id}/><input type="hidden" name="visible" value={String(!review.is_visible)}/><button className="admin-small-button">{review.is_visible ? "Ocultar" : "Publicar"}</button></form></div></article>)}{reviews.length === 0 && <p className="rounded-xl border border-dashed border-white/10 p-6 text-sm text-zinc-500">Nenhuma avaliação ainda.</p>}</div></section>
  </main>;
}
