...
export default async function CategoryPage({params}:{params:Promise<{jogoSlug:string;categoriaSlug:string}>}){
  ...
  const {data:products,error}=await client.from("products").select("id,name,slug,description,price,image_url,stock,unlimited_stock,display_order").eq("category_id",category.id).eq("is_active",true).order("display_order",{ascending:true}).order("name",{ascending:true});
